...
if grep -q "172.17.0.1/8" /data/postgresql-etc/17/main/pg_hba.conf; then
  ...
  echo "host    all             all             172.17.0.1/8            scram-sha-256" >> /etc/postgresql/17/main/pg_hba.conf
fi

if grep -q "10.17.0.1/8" /data/postgresql-etc/17/main/pg_hba.conf; then
  echo "10.17.0.1/8 exists";
else
  echo "host    all             all             10.17.0.1/8            scram-sha-256" >> /etc/postgresql/17/main/pg_hba.conf
fi
